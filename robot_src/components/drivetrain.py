...
MAX_VELOCITY = 11000  # Falcons are maxing at 20k - 21k
CLOSEDLOOPRAMP = 1.5
WHEEL_DIAMETER = 6
# TICKS_PER_INCH = 1149  #
...
class FROGDrive:
    ...
    def setup(self):
        ...
        for controller in [self.leftMaster, self.rightMaster]:
            ...
            controller.configClosedloopRamp(CLOSEDLOOPRAMP)
            controller.configSelectedFeedbackSensor(
                FeedbackDevice.IntegratedSensor, 0, 0
            )
            controller.configMotionAcceleration(MM_ACCELERATION, 0)
            controller.configMotionCruiseVelocity(MM_CRUISE_VELOCITY, 0)
            VelocityPID.configTalon(controller)
    ...
    def driveToTarget(self, x_err, y_err):
        # taking x error from center of camera (160)
        # and y error from the bottom (240)
        rotate = remap(x_err, -160, 160, -0.5, 0.5)
        speed = remap(y_err, 0, 240, 0.3, 0.8)
